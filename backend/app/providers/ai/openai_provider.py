...
Uses OpenAI GPT models for story and scene generation.
"""
...
class OpenAIProvider(AIProvider):
    ...
    Uses GPT-4 for story generation.
    """
    ...
    async def generate_story(
        ...
        """Generate a documentary-style story from a title using OpenAI GPT."""
        try:
            prompt = build_documentary_story_prompt(title, context, language, video_length)

            logger.info(f"Generating story for title: {title} using model: {self.model}")
    ...
    async def generate_scene_breakdown(
        ...
        """Break story into scenes with descriptions using OpenAI GPT."""
        try:
            prompt = build_documentary_scene_prompt(story, language, video_length, characters)

            logger.info(f"Generating scene breakdown using model: {self.model}")
    ...
    async def generate_image_prompt(
        ...
        """Generate a detailed image prompt for a scene using OpenAI GPT."""
        try:
    ...
    async def extract_characters(
        ...
        """Extract character descriptions using OpenAI GPT."""
        try:
            prompt = build_documentary_character_prompt(story)

            logger.info(f"Extracting characters using model: {self.model}")
